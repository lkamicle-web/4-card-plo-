...
export const VARIANTS = {
  lite: {
    name: 'lite',
    out: 'index.html',
    regions: ['data', 'taxonomy', 'policy', 'engine', 'ring'],
    budgets: { total: 625 * 1024, app: 410 * 1024, appCore: 361 * 1024, modelCode: 57 * 1024, ring: 20 * 1024,
    ...
    budgetSource: 'modelCode raised 54K -> 56K at v4 S1 to pay for the seat-ladder generalisation of policy.mjs (V4-PLAN §2.1-2.4): measured 56,270 B = 55.0K against 53,376 B before it, cap 56K = measured + 1.9%, tighter than the +3.6% P4 held this same ceiling at and far inside D6\'s +8% bound. SHRINK-FIRST, MEASURED IN BYTES, because §2.7 predicted the deletions would pay for the additions and measured they do not: deleting N_NB / N_BL / NEST_CHAIN, both seat-name offenders and the old two-literal positionDisabled returned 470 B; the structural functions that replace them cost 757 B, ladderConstants and the nine-seat tables 534 B, constants.ladder 291 B, its derived block 229 B, the ring accessors and their threading 629 B, and the seats axis through env / nEff / widthFor / width3For / solve3bet / whyLines 924 B - 3,364 B added against 470 B removed, +2,894 B net. Trimmed 414 B BEFORE the raise, each measured: the anchor and flag prose cut from 340 B to 145 B (-195), ladderConstants\' input-keyed memo replaced by a build-once at module init (-130), SEAT_DISPLAY dropped from the ladder block (-22), seatLabel not shipped (-67). The residue is irreducible: every remaining byte is an exported name the plan\'s §2.2 table or §4 constants table requires. METHODOLOGY §9.11, measured + ~5% at the v2 phase-4 end; app raised to 388K at '
      ...
      + 'or it does not ship. V4 S3, THE RING REGISTRATION AND ITS FOUR RAISES (V4-PLAN §2.7, D12(d), D13). blocks.ring 11K is NEW and is TIGHTER THAN ITS OWN RULE: measured 10,846 B, and ceil(10,846 x 1.05) to the whole KB is 12K, so 11K = 11,264 B is one whole-KB step under the bound and still above the block - the conservative direction this repository has now taken six times. ring 20K is NEW and is the ARTIFACT budget, a top-level row and not a model.json sub-budget: data/ring.json injects as 18,620 B (18.2K) and ceil(18,620 x 1.05) to the whole KB is 20K, which is D12(d)\'s pin from above and the exact line it prints while the row is absent. appCore RAISED 360K -> 361K, and it is the row §2.7 named as the tightest: measured 369,147 B = 360.5K lite / 369,297 B = 360.6K full against 360K = 368,640 B, over by 507 B and 657 B. 361K = 369,664 B is measured + 0.1%, against a +5% bound of 379K - the smallest whole-KB step that exists. app RAISED 398K -> 410K, and the number is ARITHMETIC rather than a judgement: the cap-sum equality (test/variant.test.mjs:280, :285) makes app = appCore + Σ(block caps), and 361 + (11+12+4+5+6+11 = 49) = 410. Measured app is 405.6K, so it would have fitted under 406K; the caps must together fit inside app - appCore, which is what the caps were added for. total RAISED 600K -> 625K, AND §2.7\'s PREDICTION THAT LITE WOULD FIT IS FALSIFIED, recorded not patched: the ring block plus an 18.2K injected payload takes lite to 633,260 B = 618.4K against 600K, over by 18,860 B. 625K = 640,000 B is measured + 1.10%, against a +5% bound of 650K. modelCode is NOT raised - five policy deltas took it to 57,175 B = 55.8K, 169 B under the 56K S1 paid for (S4\'s red-team dispositions added 23 B of flag prose naming baseRRule: 57,198 B, 146 B under, and the ceiling does not move for it). blocks.skill is NOT raised (the twelve nine-seat skill exception entries are records in scripts/lib/skill.mjs, not page bytes), core is NOT raised, and no pre-existing block cap gains one byte. SHRINK-FIRST, MEASURED IN BYTES BEFORE ANY OF IT WAS ASKED FOR (rule R6), 1,257 B recovered across three lanes and this one: S3 cut the ring region\'s shipped HTML comment from 950 B to 242 B (-708) by putting the reasoning in variant.mjs\'s ringInjectedForm docstring, which is build-time and never ships - the eq region\'s own one-line form, taken deliberately; lane U recovered 308 B in four measured steps (solveState through stateOf -83, the local POL in seatLabel/extrapReach -94, one env literal in the page instead of two -92, seatLabel moved out of shared code into @block:ring -39); lane R shrank its own SIM_NMAX / validEqArray / p.nMax cut from +614 B to +373 B (-241). S1\'s shell-range rewrite RETURNED NOTHING to spend: §2.7 predicted the duplicate POSITIONS / N_NB / N_BL / legalPos copies would pay ~0.4 KB and it measured +0.2 K NET (docs/spikes/V4-ladder.md §11) - the prediction is recorded falsified, not patched. Nothing further is available without moving pre-existing bytes into a marked block, which is laundering rather than shrinking. TWO SHRINKS ON THE ARTIFACT ITSELF WERE MEASURED AND DECLINED, with their reasons: rendering data/ring.json\'s vDelta as an array of five pairs indexed by meta.v saves a measured 3,075 B (16.5% of the artifact) but changes the payload shape docs/spikes/V4-ladder.md fixes as the accessor\'s contract, and re-cutting it costs a 1,255 s regeneration of a byte-identical measurement; dropping the note and agree fields (1,740 B) would take the per-cell agreement record D12(b)/(c) get their teeth from off the artifact. A ceiling is not worth either. V4 S5, ONE MORE RAISE AND IT IS THE DOCUMENTS\' OWN: modelCode RAISED 56K -> 57K to pay for METHODOLOGY limitation 20 shipping as DATA rather than as page prose. constants.limitations gains its third entry (the nine-seat early-seat constants are extrapolated from the six-seat ladder, no nine-handed corpus has touched them), which the Method view renders from model.constants like limitations 16 and 17 and which gates I41/I42\'s idiom byte-compares against docs/METHODOLOGY.md. Measured: the entry costs 267 B of minified policy.mjs and takes modelCode 57,198 B -> 57,465 B against 56K = 57,344 B, over by 121 B. 57K = 58,368 B is measured + 1.57%, against D6\'s calibrated bound of 62K. SHRINK-FIRST, MEASURED IN BYTES, and it did not pay: the entry is already the shortest form that carries the admission - joining the note\'s two source chunks into one literal returns 3 B, trimming `of` to a phrase returns 6 B, and dropping `flagsItExplains` (already dropped, for metaCore\'s sake) returns 38 B of model payload and 0 B of modelCode, so 47 B against a 121 B overrun. Deleting words from the admission to fit a ceiling is choosing the answer, which this repository has ruled on twice. The 263 B the same entry costs data/model.json are NOT a raise: they are spent out of metaCore\'s own headroom, which reads 13,300 B of 13,312 after it - 12 B, now the tightest number in the payload, and the next constant that wants a sentence needs the reserved sub-budget idiom rather than that gap. ',
  ...
  full: {
    name: 'full',
    out: 'index-full.html',
    regions: ['data', 'taxonomy', 'policy', 'engine', 'eq', 'ring'],
    budgets: { total: 695 * 1024, app: 410 * 1024, appCore: 361 * 1024, modelCode: 57 * 1024, eq: 73 * 1024, ring: 20 * 1024,
      blocks: { gto: 11 * 1024, ev: 12 * 1024, skill: 4 * 1024, topn: 5 * 1024, calib: 6 * 1024, ring: 11 * 1024 } },
    budgetSource: 'modelCode raised 54K -> 56K at v4 S1 to pay for the seat-ladder generalisation of policy.mjs (V4-PLAN §2.1-2.4): measured 56,270 B = 55.0K against 53,376 B before it, cap 56K = measured + 1.9%, tighter than the +3.6% P4 held this same ceiling at and far inside D6\'s +8% bound. SHRINK-FIRST, MEASURED IN BYTES, because §2.7 predicted the deletions would pay for the additions and measured they do not: deleting N_NB / N_BL / NEST_CHAIN, both seat-name offenders and the old two-literal positionDisabled returned 470 B; the structural functions that replace them cost 757 B, ladderConstants and the nine-seat tables 534 B, constants.ladder 291 B, its derived block 229 B, the ring accessors and their threading 629 B, and the seats axis through env / nEff / widthFor / width3For / solve3bet / whyLines 924 B - 3,364 B added against 470 B removed, +2,894 B net. Trimmed 414 B BEFORE the raise, each measured: the anchor and flag prose cut from 340 B to 145 B (-195), ladderConstants\' input-keyed memo replaced by a build-once at module init (-130), SEAT_DISPLAY dropped from the ladder block (-22), seatLabel not shipped (-67). The residue is irreducible: every remaining byte is an exported name the plan\'s §2.2 table or §4 constants table requires. D9, P3, both HELD BELOW measured+5% rather than re-derived up to it (the P3 red '
      ...
      + '2.7% of headroom left. V4 S3: total RAISED 660K -> 695K, the raise §2.7 EXPECTED rather than feared and the only one on this row that was predicted. The full page carries the ring block AND the 18,620 B injected payload on top of the equilibrium: measured 704,433 B = 687.9K against 660K = 675,840 B, over by 28,593 B. 695K = 711,680 B is measured + 1.03%, against a +5% bound of 723K - held below its own rule for the fourth time on this row, the reading P3 settled and P4 and P5 re-applied. eq is UNTOUCHED at 73K: data/equilibrium.json moved +22 B this run (lane K\'s MULTIWAY_DEFERRAL rename in two notes strings) and reads 69.6K, 4.9% under. appCore RAISED 360K -> 361K, app RAISED 398K -> 410K, blocks.ring 11K and ring 20K follow lite\'s numbers and lite\'s reasoning, because they are the same code and the same artifact in both variants - full measures 150 B over lite on appCore, one script wrapper and one bridge line, exactly as it always has. SHRINK-FIRST, MEASURED IN BYTES BEFORE ANY OF IT WAS ASKED FOR (rule R6), 1,257 B recovered across three lanes and this one: S3 cut the ring region\'s shipped HTML comment from 950 B to 242 B (-708) by putting the reasoning in variant.mjs\'s ringInjectedForm docstring, which is build-time and never ships - the eq region\'s own one-line form, taken deliberately; lane U recovered 308 B in four measured steps (solveState through stateOf -83, the local POL in seatLabel/extrapReach -94, one env literal in the page instead of two -92, seatLabel moved out of shared code into @block:ring -39); lane R shrank its own SIM_NMAX / validEqArray / p.nMax cut from +614 B to +373 B (-241). S1\'s shell-range rewrite RETURNED NOTHING to spend: §2.7 predicted the duplicate POSITIONS / N_NB / N_BL / legalPos copies would pay ~0.4 KB and it measured +0.2 K NET (docs/spikes/V4-ladder.md §11) - the prediction is recorded falsified, not patched. Nothing further is available without moving pre-existing bytes into a marked block, which is laundering rather than shrinking. TWO SHRINKS ON THE ARTIFACT ITSELF WERE MEASURED AND DECLINED, with their reasons: rendering data/ring.json\'s vDelta as an array of five pairs indexed by meta.v saves a measured 3,075 B (16.5% of the artifact) but changes the payload shape docs/spikes/V4-ladder.md fixes as the accessor\'s contract, and re-cutting it costs a 1,255 s regeneration of a byte-identical measurement; dropping the note and agree fields (1,740 B) would take the per-cell agreement record D12(b)/(c) get their teeth from off the artifact. A ceiling is not worth either. V4 S5, ONE MORE RAISE AND IT IS THE DOCUMENTS\' OWN: modelCode RAISED 56K -> 57K to pay for METHODOLOGY limitation 20 shipping as DATA rather than as page prose. constants.limitations gains its third entry (the nine-seat early-seat constants are extrapolated from the six-seat ladder, no nine-handed corpus has touched them), which the Method view renders from model.constants like limitations 16 and 17 and which gates I41/I42\'s idiom byte-compares against docs/METHODOLOGY.md. Measured: the entry costs 267 B of minified policy.mjs and takes modelCode 57,198 B -> 57,465 B against 56K = 57,344 B, over by 121 B. 57K = 58,368 B is measured + 1.57%, against D6\'s calibrated bound of 62K. SHRINK-FIRST, MEASURED IN BYTES, and it did not pay: the entry is already the shortest form that carries the admission - joining the note\'s two source chunks into one literal returns 3 B, trimming `of` to a phrase returns 6 B, and dropping `flagsItExplains` (already dropped, for metaCore\'s sake) returns 38 B of model payload and 0 B of modelCode, so 47 B against a 121 B overrun. Deleting words from the admission to fit a ceiling is choosing the answer, which this repository has ruled on twice. The 263 B the same entry costs data/model.json are NOT a raise: they are spent out of metaCore\'s own headroom, which reads 13,300 B of 13,312 after it - 12 B, now the tightest number in the payload, and the next constant that wants a sentence needs the reserved sub-budget idiom rather than that gap. ',
...
/**
 * THE RING'S INJECTED FORM — ONE definition, used by the build that emits it and by the gate that
 * budgets it (V4-PLAN §2.4, D12(d)). Two copies of this string is the defect the pin exists to
 * prevent: a build emitting `MODEL.ring = …` while D12(d) measures `const RING = …` would bound a
 * payload the artifact does not carry, and the ceiling would be honest about the wrong number.
 *
 * IT ASSIGNS, IT DOES NOT DECLARE, and that is not a style choice. `src/shell.html`'s main script
 * declares `var RING = null` at its own top level: a global `const RING` in the injected script is
 * a SyntaxError against that `var`, and a `window.RING = …` is nulled by the `var` before the page
 * reads it. `MODEL.ring` is the seam docs/spikes/V4-ladder.md specified and the one the shell
 * already reads. The region sits after `@inject:data`, which is what declares `MODEL`.
 *
 * It lives HERE and not in `scripts/lib/ring.mjs` because that file's bytes are hashed into
 * `data/ring.json`'s `meta.generatorHash`: a line added there reddens D12(a) and costs a 21-minute
 * regeneration of an artifact that has not changed. This module is not hashed by anything.
 *
 * @param {string} raw the bytes of data/ring.json
 * @returns {string} exactly what `scripts/build.mjs` splices between the ring markers
 */
export const ringInjectedForm = (raw) => `MODEL.ring = ${JSON.stringify(JSON.parse(raw))};`;
