...
const P3_ADJUDICATED = [
  ...
  "(2) S-A'S ANCHORS BIND: epsilon 5e-5 bb; iteration cap 2,000; two-seed clause 0.15% of pot; 12.5k-25k boards; the 3/9/27/81 ladder is an identity (zero constants); I35's cap list bounds the omissions (no limp, no sixth raise, no postflop).",
...
async function runP3(state) {
  ...
  const pre = await agent([
    HOUSE,
    "",
    "TASK - P3 precheck (barrier B2, under S-B Grade C). Verify, changing nothing: (1) scripts/lib/payoff-model.mjs exists with ENABLED === false and scripts/lib/cfr.mjs exists; (2) the tree is GREEN at HEAD: `node scripts/verify.mjs` all 53 gates pass incl. I22, I32, I33 (six keys, clauses (a)-(h) + monotonicity) and I35; `node --test test/*.test.mjs` 519/519; `node scripts/build.mjs --check` reports lite current and full SKIPPED BY NAME (no data/equilibrium.json yet - that is expected here and only here); (3) `node smoke.mjs` green with its three slider-morph rows (floor 8 ms, ON 16 ms, OFF 4 ms); (4) `git status` clean and `git worktree list` shows the S-A checkout worktree-wf_5a8a2571-726-2 (or `git show worktree-wf_5a8a2571-726-2:scripts/spikes/sa-matrix.mjs` prints the file); (5) read docs/spikes/S-B.md's grade band (Grade C: p95 > 5.0) and docs/spikes/S-A.md's verdict. THE B2 CONDITION - I33 passing on a non-stub source - is NOT yours to assert: under Grade C there is no `source:'model'`, and the B2 pre-stage that follows you builds the measured pairwise matrix and runs I33 on it. Report Grade C explicitly.",
    "Return JSON: ok (all five hold), detail (S-A verdict, the verify/test/check/smoke readings, anything failing), gradeBand ('C' expected)."
