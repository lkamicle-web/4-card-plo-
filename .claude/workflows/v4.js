...
async function runS2(state, s1) {
  ...
  for (const l of lanes) {
    ...
    if (typeof l.ringWallSec === "number") {
      ...
      log("Ring: " + l.ringWallSec + " s against the pre-registered budget (300 s as written; 1,280 s after the owner's R2 amendment); worst prefix delta " + l.ringPrefixWorstSE + " x se.cell (band 2.0); " + l.ringBytes + " B");
    }
  }
...
async function runS5(state, s1, s3) {
  ...
  const r = await agent([
    ...
    "2. README.md - 'What v4 added' after 'What v3 added'; backlog item 16 CLOSED with the 7-max/8-max residue restated as the deliberate out-of-scope decision it is (section 0.2); 'Regenerating the data' gains scripts/generate-ring.mjs with its own wall budget (1,280 s after the owner's R2 amendment; the 300 s derivation it replaced is recorded as falsified), stated as separate from generate-data.mjs's 188 s; 'Repo layout' corrected for the dual build (an open v3.1 item, repaired here because the section is being edited anyway).",
    "3. docs/V4-PLAN.md annotated IN PLACE in the V2-PLAN/V3-PLAN idiom: `> **Measured (stage Sn).**` blocks beneath the predictions this run confirmed OR FALSIFIED - the legal-pair count, the fixture settings count against 26,136, the ring wall time against the 300 s as written AND the owner-amended 1,280 s (the two 'Amended by the owner' blocks are the owner's and stay as written; your Measured block goes beneath them), D12(c)'s prefix agreement, R1's decision with both rules' readings, R3's census, I50's rfi containment (equality measured, or a strict superset with its per-seat union counts), and every byte raise - the full variant's total above all. Plan text above each annotation is kept AS WRITTEN; reversals are recorded, never edited away. Append the section 10 resolutions: questions 1, 2 and 3 answered with the evidence that answered them.",
